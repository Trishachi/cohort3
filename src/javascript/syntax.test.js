...
test("Which number is bigger?", () => {
  ...
  expect(functions.biggerNum(2, 1)).toBe("2 is bigger");
});
...
test("revsers the given array", () => {
  expect(functions.reverseArray([1, 3, 6, 9, 5, 2, 8, 5])).toEqual([
    5,
    8,
    2,
    5,
...
test("Is task in Todo List?", () => {
  expect(
    functions.updateTodo("code", ["code", "laundry", "dinner"], "sleep")
  ).toEqual(["sleep", "laundry", "dinner"]);
  expect(
    functions.updateTodo("eat", ["code", "laundry", "dinner"], "sleep")
  ).toEqual(["eat", "code", "laundry", "dinner"]);
});

test("Find Profile", () => {
  expect(functions.profileLookup("Harry", "likes")).toEqual([
    "Hogwarts",
    "Magic",
    "Hagrid"
  ]);
  expect(functions.profileLookup("Harry", "dislikes")).toBe(
    "Property does not exist"
  );
  expect(functions.profileLookup("Jason", "likes")).toBe(
...
test("Print profile details", () => {
  expect(functions.profileDetails()).toEqual(
